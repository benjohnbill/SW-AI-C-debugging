...
# ~/.gdbinit, ~/.config/gdb, ~/.gdb_history : 호스트의 gdb 설정과 명령 히스토리를 그 HOME 에
...
DOCKER := docker run --rm $(DOCKER_TTY) \
  ...
  -v "$(HOME)/.gdbinit":/tmp/.gdbinit:ro -v "$(HOME)/.config/gdb":/tmp/.config/gdb:ro -v "$(HOME)/.gdb_history":/tmp/.gdb_history \
